...
기준 브랜치/커밋: `main` · `8dd4c71 fix: keep atlas scrolling dark and vertical`
...
| 항목 | 현재 상태 |
|---|---|
| Git | `main`은 `github/main`과 동기화됨. 이 문서 작성으로만 새 uncommitted 파일이 생긴다. |
| 로컬 웹 서버 | `localhost:3000` 리스너 없음. 현재 앱/런타임은 실행 중이 아니다. |
| 최근 검증 | `npm test` **193 passed**, `npm run db:baseline`, `npm run graph:audit` 통과. |
...
| 문서 | 불일치 |
|---|---|
| `README.md` | parser v4 표기, 94,488 relation / Codex 1 / 176 tests 등의 과거 수치가 남아 있다. |
...
다음 문서 작업이 필요하다면, 먼저 현 audit 수치(94,506 relation, Codex 19, tests 193)를 반영하고 parser profile 설명을 v5로 갱신한다. 데이터 자체를 바꾸지 않는 문서 갱신은 별도 commit으로 분리하는 것을 권장한다.
...
```bash
# 전체 회귀: build + server runtime TypeScript + 193 tests
npm test
...
`npm test` 로그의 의도된 failure scenario 출력(예: test fixture의 `[github-source-api] unhandled Error`)은 테스트가 이를 검증하는 과정에서 나오는 것이며, 최종 결과가 `pass 193 / fail 0`이면 정상이다.
