...
| 항목 | 현재 상태 |
|---|---|
| Git | 정적 배포 코드 기준 `3213aa5`와 이후 배포 기록 문서까지 GitHub `main` push 완료. 최신 이어받기 commit은 `git log -1`로 확인한다. |
| 로컬 웹 서버 | 정적 QA는 `npm run preview:vercel`, 전체 로컬 앱은 `npm run dev:web`을 사용한다. |
| 최근 검증 | `npm test` **206 passed**, lint·TypeScript·공개 artifact·Vercel 정적 output 검사 통과. |
| GitHub CI | 배포 기록 문서 대상 `Public Atlas static checks` 성공: <https://github.com/coreline-ai/memory_node_graph/actions/runs/31397988233> |
...
| Git 자동 배포 | `2026-08-13` Vercel Git Integration을 `coreline-ai/memory_node_graph`에 연결했다. 다음 `main` push에서 Production 자동 배포와 commit SHA 일치를 검증해야 한다. |
...
이 target은 현재 Three.js GUI만 재사용하고 Vercel Function, D1, SQLite, `/api/*`, OAuth, 환경 변수를 사용하지 않는다. Public Corpus는 500노드·1,495 실제 관계·505 display 선이고, Gold 68/101 및 Max 500/2,000은 각각 검토 표본과 합성 데모로 표시한다.

---

## 3. 현재 로컬 D1 데이터 기준선

`npm run db:baseline`과 `npm run graph:audit`를 2026-08-10 KST에 다시 실행해 확인했다.
...
| 고유 entity | 89,669 |
| 저장 relation | 94,506 |
| 규칙 relation / Codex relation | 94,487 / 19 |
| enrichment job | 10,982 |
| `queued` / `completed` / `stale` | 10,893 / 11 / 78 |
| 현재 Codex provider queued | 68 |
...
- 공개 snapshot은 현재 78개 공개 저장소·519문서에서 만든 53,377개 공개 전용 node·56,341개 저장 관계의 제한 투영이며, 원본 D1·전체 문서·evidence·OAuth/job 정보가 아니다.
...
- **실제 corpus**: D1의 89,669 entity / 94,506 relation에서 관계 중심 최대 500노드·2,000선을 화면에 투영한다.
...
4. 기존 Codex relation 19개를 삭제·복구·변경하지 않는다.
...
현재 provider queued 68개를 읽기 전용 재점수화한 결과는 아래와 같다.

| high | review | excluded | resolved pair |
|---:|---:|---:|---:|
| 0 | 64 | 4 | 0 |

앵커는 53개 중 21개만 현재 job input node로 해석됐고 32개는 미해결이다. 이것은 점수 임계값 문제가 아니라 **현 D1이 parser v4로 이미 생성된 상태**이기 때문이다. 코드의 v5 parser는 다음 승인된 reindex/new job부터 적용된다.
...
## 7. 알려진 문서 불일치와 정리 우선순위

현재 code/D1 audit을 기준으로 다음 문서는 **역사 기록이 포함되어 수치가 오래됐다.** 구현 판단에는 아래 handoff의 최신 기준선을 우선한다.

| 문서 | 불일치 |
|---|---|
| `README.md` | 2026-08-07 D1 수량 표는 역사 기준선이다. 테스트 수와 공개 snapshot 수량은 최신화했다. |
| `docs/current-oauth-runtime.md` | 94,488 relation 및 과거 fingerprint가 남아 있다. |
| `docs/local-d1-baseline.md` | 과거 baseline 기록용 문서다. 실제 수치는 항상 `npm run db:baseline` 출력으로 확인한다. |

다음 D1 기준선 문서 작업이 필요하다면, 현 audit 수치(94,506 relation, Codex 19)를 반영하고 parser profile 설명을 v5로 갱신한다. 데이터 자체를 바꾸지 않는 문서 갱신은 별도 commit으로 분리하는 것을 권장한다.
...
```bash
# 전체 회귀: build + server runtime TypeScript + 206 tests
npm test
...
`npm test` 로그의 의도된 failure scenario 출력(예: test fixture의 `[github-source-api] unhandled Error`)은 테스트가 이를 검증하는 과정에서 나오는 것이며, 최종 결과가 `pass 206 / fail 0`이면 정상이다.
